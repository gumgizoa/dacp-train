...
# Showcase how to use fsdp with memory tracker
from accelerate import Accelerator, DistributedType
from accelerate import FullyShardedDataParallelPlugin, DataLoaderConfiguration
...
class TorchTracemalloc:
    def __enter__(self):
        ...
        if torch.cuda.is_available():
            torch.cuda.empty_cache() # release all unused memory from previous CUDA operations
            torch.cuda.reset_max_memory_allocated()  # reset the peak gauge to zero
            self.begin = torch.cuda.memory_allocated() # record the current memory allocated
...
def training_function(config, args):
    ...
    # To see how use_seedable_sampler works, check use_seedable_sampler.py script on the same path
    dataloader_config = DataLoaderConfiguration(use_seedable_sampler=args.use_seedable_sampler)
    accelerator = Accelerator(
        cpu=args.cpu,
    ...
    for epoch in range(starting_epoch, num_epochs):
        # Context manager to track the memory usage during training
        with TorchTracemalloc() as tracemalloc:
            ...
            for step, batch in enumerate(active_dataloader):
                ...
                # If mixed precision was used, will also save a "scaler.bin" file
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="If the dataloader should be a seedable dataloader.",
    )
